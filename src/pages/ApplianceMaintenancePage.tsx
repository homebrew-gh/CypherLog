...
import { useMaintenanceByAppliance, calculateNextDueDate, formatDueDate, isOverdue, isDueSoon } from '@/hooks/useMaintenance';
...
export function ApplianceMaintenancePage() {
  const { applianceId } = useParams<{ applianceId: string }>();
  const navigate = useNavigate();
  const { user } = useCurrentUser();
  
  const appliance = useApplianceById(applianceId);
  const maintenance = useMaintenanceByAppliance(applianceId);
  ...
  const { scheduledMaintenance, logOnlyMaintenance, allMaintenanceWithDates } = useMemo(() => {
    ...
    const withDates = scheduled.map(m => {
      ...
      const nextDue = calculateNextDueDate(purchaseDate, m.frequency, m.frequencyUnit, lastCompletion?.completedDate);
      const overdue = isOverdue(purchaseDate, m.frequency, m.frequencyUnit, lastCompletion?.completedDate);
      const dueSoon = isDueSoon(purchaseDate, m.frequency, m.frequencyUnit, lastCompletion?.completedDate);
  ...
  }, [maintenance, allCompletions, appliance?.purchaseDate]);

  // Get all completions for this appliance's maintenance
  const applianceCompletions = useMemo(() => {
        ...
        <div className="container mx-auto px-4 py-8">
          <Card className="border-dashed">
            <CardContent className="py-12 text-center">
              <p className="text-muted-foreground">Please log in to view appliance maintenance.</p>
            </CardContent>
          </Card>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-theme-gradient">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-foreground">{appliance.model}</h1>
                <p className="text-sm text-muted-foreground">
                  Maintenance History
                  {appliance.room && <span className="ml-2">({appliance.room})</span>}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  Recurring maintenance tasks for this appliance
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  All completed maintenance for this appliance
                </CardDescription>
              </CardHeader>
              <CardContent>
                {applianceCompletions.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                  ...
                  <ScrollArea className="h-[400px] pr-4">
                    <div className="space-y-3">
                      {applianceCompletions.map(completion => {
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-green-600">{applianceCompletions.length}</span>
                  </div>
                  <Separator />
                  <div className="flex justify-between items-center">
      ...
      <MaintenanceDialog
        ...
        mode="appliance"
        preselectedApplianceId={applianceId}
      />
